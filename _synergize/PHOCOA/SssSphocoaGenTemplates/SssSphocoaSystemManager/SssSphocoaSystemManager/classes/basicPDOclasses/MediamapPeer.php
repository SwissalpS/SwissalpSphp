...
require 'basicPDOclasses/om/BaseMediamapPeer.php';
 ...
 */
class MediamapPeer extends BaseMediamapPeer {
...
//WFLog::log($sPath . '.' . $sExt);
			...
			if (is_file($sPath . '.' . $sExt)) return $sPath . '.' . $sExt;
...
//WFLog::log($sPath . '.' . $sExt);
	...
	static function imgBigOrSmallerForBridgeUID($iUID) {
		...
		$aNotes = NotesPeer::notesPopularThumbnailsForBridgeUID($iUID, 1);

		if (empty($aNotes)) return array();

		$oMediaMap = MediamapPeer::retrieveByPK($aNotes[0]->getMediauid());
