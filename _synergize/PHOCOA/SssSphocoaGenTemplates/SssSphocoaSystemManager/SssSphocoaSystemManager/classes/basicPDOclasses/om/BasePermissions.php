...
abstract class BasePermissions extends BaseObject  implements Persistent {
	...
	public function setUid($v)
	{
		...
		if ($this->uid !== $v) {
			$this->uid = $v;
			$this->modifiedColumns[] = PermissionsPeer::UID;
		}
	...
	public function setDomain($v)
	{
		...
		if ($this->domain !== $v || $v === 'GLOBAL') {
			$this->domain = $v;
			$this->modifiedColumns[] = PermissionsPeer::DOMAIN;
		}
	...
	public function setHash($v)
	{
		...
		if ($this->hash !== $v) {
			$this->hash = $v;
			$this->modifiedColumns[] = PermissionsPeer::HASH;
		}
	...
	public function hasOnlyDefaultValues()
	{
			...
			if (array_diff($this->modifiedColumns, array(PermissionsPeer::DOMAIN))) {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->hash = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			$this->resetModified();
			...
			return $startcol + 3; // 3 = PermissionsPeer::NUM_COLUMNS - PermissionsPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PermissionsPeer::UID)) $criteria->add(PermissionsPeer::UID, $this->uid);
		if ($this->isColumnModified(PermissionsPeer::DOMAIN)) $criteria->add(PermissionsPeer::DOMAIN, $this->domain);
		if ($this->isColumnModified(PermissionsPeer::HASH)) $criteria->add(PermissionsPeer::HASH, $this->hash);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(PermissionsPeer::UID, $this->uid);
